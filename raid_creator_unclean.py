...
if len(argv) == 2:
    ...
    dpl=14
print("You have chosen",dpl,"disks per RAID LUN. If this is not right, ctrl-c now.") 
input()
...
def createRAID6Array(partitions,raidname):
    ...
    if Path('/bitmap').is_dir():
        command = ['mdadm','-v','--create','/dev/'+raidname,'--level=6','--assume-clean','--raid-devices='+str(numDisks),'--bitmap=/bitmap/'+raidname]
    else:
        command = ['mdadm','-v','--create','/dev/'+raidname,'--level=6','--assume-clean','--raid-devices='+str(numDisks)]
    for part in partitions:
